...
begin
  ...
  while not cdsCadastro.Eof do
    ...
    wTexto:= wTexto + #13 + 'Código: ' + cdsCadastrobdCodigo.AsString + ' Nome: ' + cdsCadastrobdNome.AsString + ' CPF: ' + cdsCadastrobdCPF.AsString +  ' Data de Nascimento: ' + cdsCadastrobdData.AsString
...
procedure TForm1.edCodigoExit(Sender: TObject);  //Rotina pra preencher automaticamente os campos a partir do CÓDIGO do usuário.
begin
cdsCadastro.IndexFieldNames := 'bdCodigo'; //Seta a procura nessa coluna da tabela
  if cdsCadastro.FindKey([edCodigo.Text]) then //(FindKey) método pra procurar a informação digitada do edit (edCodigo.Text) na coluna setada anteriormente.
     begin
       edNome.Text:= cdsCadastrobdNome.AsString;
       edCPF.Text:= cdsCadastrobdCPF.AsString;
       edData.Text:= cdsCadastrobdData.AsString;
       cbEstado.ItemIndex:= cdsCadastrobdEstado.AsInteger;
       edTelefone.Text:= cdsCadastrobdTelefone.AsString;
       edEndereco.Text:= cdsCadastrobdEndereco.AsString;
     end
  else                       //Caso o CÓDIGO digitado não exista, esse método deixa todos os campos vazios.
     begin
       edNome.Text:= '';
       edCPF.Text:= '';
       edData.Text:= '';
